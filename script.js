...
function simulatorTemplate(type) {
  const controls = {
    linear: `${control('데이터 개수', 'linear-count-value', 'linear-count')}${control('잡음 정도', 'linear-noise-value', 'linear-noise')}<button class="action-button" onclick="resetLinear()">새 데이터 만들기</button>`,
...
function setupCanvas(id) {
  ...
  canvas.width = rect.width * ratio; canvas.height = 430 * ratio;
  const ctx = canvas.getContext('2d'); ctx.scale(ratio, ratio);
  return { canvas, ctx, width: rect.width, height: 430 };
}
...
function initLinear() {
  ...
  const generate = () => { points.length = 0; const count = +get('linear-count').value; const noise = +get('linear-noise').value; for (let index = 0; index < count; index++) points.push({ x: 35 + index / (count - 1) * (sim.width - 70), y: sim.height - 45 - index / (count - 1) * 260 + (Math.random() - .5) * noise }); draw(); };
...
function initLogistic() {
  const sim = setupCanvas('logistic-canvas'); const points = [];
  const draw = () => { const slope = +get('logistic-slope').value; const bias = +get('logistic-bias').value; const threshold = +get('logistic-threshold').value; sim.ctx.clearRect(0, 0, sim.width, sim.height); drawGrid(sim.ctx, sim.width, sim.height); const boundary = sim.height / 2 - (bias / slope || 0) * 70; sim.ctx.strokeStyle = '#ea6048'; sim.ctx.setLineDash([7, 5]); sim.ctx.beginPath(); sim.ctx.moveTo(0, boundary); sim.ctx.lineTo(sim.width, boundary); sim.ctx.stroke(); sim.ctx.setLineDash([]); let correct = 0; points.forEach(point => { const probability = 1 / (1 + Math.exp(-(slope * (sim.height - point.y) / sim.height + bias))); if ((probability >= threshold ? 1 : 0) === point.label) correct++; sim.ctx.fillStyle = point.label ? '#ea6048' : '#2563eb'; sim.ctx.beginPath(); sim.ctx.arc(point.x, point.y, 7, 0, Math.PI * 2); sim.ctx.fill(); }); get('logistic-boundary').textContent = `y = ${boundary.toFixed(0)}`; get('logistic-accuracy').textContent = `${Math.round(correct / points.length * 100)}%`; get('logistic-result').textContent = threshold.toFixed(2); };
  const generate = () => { points.length = 0; for (let index = 0; index < 24; index++) points.push({ x: 50 + Math.random() * (sim.width - 100), y: index < 12 ? sim.height * .3 + Math.random() * 100 : sim.height * .6 + Math.random() * 100, label: index < 12 ? 1 : 0 }); draw(); };
...
function initKmeans() {
  ...
  const draw = () => { sim.ctx.clearRect(0, 0, sim.width, sim.height); drawGrid(sim.ctx, sim.width, sim.height); points.forEach(point => { sim.ctx.fillStyle = colors[point.cluster % colors.length]; sim.ctx.beginPath(); sim.ctx.arc(point.x, point.y, 6, 0, Math.PI * 2); sim.ctx.fill(); }); centers.forEach((center, index) => { sim.ctx.strokeStyle = colors[index]; sim.ctx.lineWidth = 4; sim.ctx.beginPath(); sim.ctx.arc(center.x, center.y, 12, 0, Math.PI * 2); sim.ctx.stroke(); }); get('kmeans-result').textContent = `${centers.length}개`; };
...
function initSvm() {
  ...
  const reset = () => { points.length = 0; for (let index = 0; index < 8; index++) points.push({ x: sim.width * (index < 4 ? .22 + .07 * index : .62 + .07 * (index - 4)), y: sim.height * (index < 4 ? .25 + .1 * (index % 2) : .65 + .1 * (index % 2)), group: index < 4 ? 1 : -1 }); draw(); };
