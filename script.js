...
function renderStepList(id, title, steps) {
  const container = get(id);
  if (!container) return;

  let list = container.querySelector(`#${id}-steps`);
  if (!list) {
    const heading = container.querySelector('strong') || document.createElement('strong');
    if (!container.querySelector('strong')) {
      container.insertAdjacentHTML('beforeend', `<strong>${title}</strong>`);
  ...
  } else if (container.querySelector('strong') && container.querySelector('strong').textContent !== title) {
...
function initLinear() {
  ...
  const draw = () => {
    ...
    if (!coordinateText) {
      const learningPanel = get('linear-learning');
      if (learningPanel) learningPanel.insertAdjacentHTML('beforeend', '<span id="linear-coordinate">점 위에 마우스를 올리면 정확한 좌표가 표시됩니다.</span>');
    } else {
      coordinateText.textContent = '점 위에 마우스를 올리면 정확한 좌표가 표시됩니다.';
    }
  };
