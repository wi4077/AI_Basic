...
function initLinear() {
  ...
  const toDataPoint = point => ({ x: point.x / sim.width * 10, y: (sim.height - point.y) / sim.height * 10 });
  const drawLinearGrid = () => { sim.ctx.strokeStyle = '#e5e1d8'; sim.ctx.lineWidth = 1; sim.ctx.font = '12px sans-serif'; sim.ctx.fillStyle = '#6b7684'; sim.ctx.textAlign = 'center'; for (let index = 0; index <= 10; index++) { const x = index / 10 * sim.width; const y = sim.height - index / 10 * sim.height; sim.ctx.beginPath(); sim.ctx.moveTo(x, 0); sim.ctx.lineTo(x, sim.height); sim.ctx.stroke(); sim.ctx.fillText(index, x, sim.height - 8); sim.ctx.textAlign = 'right'; sim.ctx.fillText(index, Math.max(x - 6, 24), Math.max(y - 5, 14)); sim.ctx.textAlign = 'center'; } sim.ctx.strokeStyle = '#9aa4b2'; sim.ctx.lineWidth = 1.5; sim.ctx.beginPath(); sim.ctx.moveTo(0, sim.height); sim.ctx.lineTo(sim.width, sim.height); sim.ctx.moveTo(0, 0); sim.ctx.lineTo(0, sim.height); sim.ctx.stroke(); };
  const draw = () => {
